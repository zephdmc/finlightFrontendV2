...
const MembersPage = () => {
  ...
  // Define isSubRoute BEFORE using it in useEffect
  ...
  // Check member limit when on members page (not subroute)
  useEffect(() => {
    if (isAdmin && !isSubRoute) {
      checkMemberLimit();
      fetchMembers();
    }
  }, [isAdmin, isSubRoute]);
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      // Extract members from the response based on your API structure
      let membersData = [];
      if (response.data.data?.records) {
        membersData = response.data.data.records;
      ...
      // Don't show toast error for this as it's not critical for page functionality
      setMembers([]);
    } finally {
  ...
  const handlePrint = () => {
    ...
    const tableRows = members.map((member, index) => {
      ...
      const hasPaid = member.registrationStatus === 'paid' || member.hasPaidRegistration;
      ...
      <tr>
        ...
        <td style="padding: 10px; border: 1px solid #ddd; text-align: center;">${member.memberId || 'N/A'}</td>
        <td style="padding: 10px; border: 1px solid #ddd; text-align: center;">${new Date(member.createdAt).toLocaleDateString()}</td>
    ...
    const paidCount = members.filter(m => m.registrationStatus === 'paid' || m.hasPaidRegistration).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1">
              ...
              {!isSubRoute && (
                <Link
                  to="/members/add"
                  className={`inline-flex items-center gap-2 mt-4 px-4 py-2 backdrop-blur-sm rounded-xl transition-all text-sm font-medium ${
                    memberLimit?.isFull 
                      ? 'bg-gray-400 cursor-not-allowed opacity-50' 
                      : 'bg-white/20 hover:bg-white/30'
                  }`}
                  onClick={(e) => {
                    if (memberLimit?.isFull) {
                      e.preventDefault();
                ...
                >
                  ...
                  Add New Member
                  {memberLimit && !memberLimit.isFull && (
                    <span className="ml-1 text-xs bg-white/30 px-1.5 py-0.5 rounded-full">
                      {memberLimit.availableSlots} slots left
                    </span>
                  )}
                </Link>
              )}
      ...
      {!isSubRoute && memberLimit && (memberLimit.isFull || showLimitWarning) && (
        <div className={`max-w-7xl mx-auto px-4 mt-4 ${
          memberLimit.isFull ? 'bg-red-50 border border-red-200' : 'bg-yellow-50 border border-yellow-200'
        } rounded-xl p-3`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className={`text-sm font-medium ${
                memberLimit.isFull ? 'text-red-800' : 'text-yellow-800'
              }`}>
                {memberLimit.isFull 
              ...
              <p className={`text-xs ${
                memberLimit.isFull ? 'text-red-700' : 'text-yellow-700'
              } mt-0.5`}>
                {memberLimit.isFull
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
        <Routes>
          ...
          <Route path="edit/:id" element={<MemberForm isEdit={true} />} />
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
